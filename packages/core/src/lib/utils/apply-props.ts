...
export function applyProps<TInstance extends object>(
  ...
  const localState = instanceLocalState(instance) || ({} as NgtInstanceLocalState);
  const rootState = localState.store?.getState();
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (value === DEFAULT + 'remove') {
      if (targetProp && targetProp.constructor) {
        ...
        const dummyInstance = new (currentInstance.constructor as NgtAnyConstructor)(
          ...((instanceLocalState(currentInstance)?.memoized?.['args'] as unknown[]) || [])
        );
    ...
    if (
      ...
      // Test again target.copy(class) next ...
        ...
        if (!('ColorManagement' in THREE) && !rootState?.linear && isColor) {
