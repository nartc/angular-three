...
export function removeInteractivity(store: StoreApi<NgtState>, object: THREE.Object3D) {
...
export function createEvents(store: StoreApi<NgtState>) {
  /** Calculates delta */
  ...
  function intersect(event: NgtDomEvent, filter?: (objects: THREE.Object3D[]) => THREE.Object3D[]) {
    ...
    for (let i = 0; i < eventsObjects.length; i++) {
      const instanceState = instanceLocalState(eventsObjects[i])?.store.getState();
      if (instanceState) {
    ...
    function handleRaycast(obj: THREE.Object3D<THREE.Event>) {
      const state = instanceLocalState(obj)?.store.getState();
    ...
    let hits: THREE.Intersection<THREE.Object3D<THREE.Event>>[] = eventsObjects
      ...
      .sort((a, b) => {
        const aState = instanceLocalState(a.object)?.store.getState();
        const bState = instanceLocalState(b.object)?.store.getState();
  ...
  function handleIntersects(
    ...
    if (intersections.length) {
      ...
      for (const hit of intersections) {
        const state = instanceLocalState(hit.object)?.store.getState() || rootState;
