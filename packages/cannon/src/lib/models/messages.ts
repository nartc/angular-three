...
import { WorkerCollideBeginEvent, WorkerCollideEndEvent, WorkerCollideEvent, WorkerRayhitEvent } from './events';
import { Observation } from './observation';
import { NgtPhysicsStoreState } from './physics-state';
...
type AddConstraintMessage = WithUUID<'addConstraint',
...
type SetConstraintMotorMaxForce = WithUUID<'setConstraintMotorMaxForce',
  number>;
...
type AddSpringMessage = WithUUID<'addSpring',
...
type AddRaycastVehicleMessage = WithUUIDs<'addRaycastVehicle',
  [
...
type ApplyRaycastVehicleEngineForceMessage = WithUUID<'applyRaycastVehicleEngineForce',
  [value: number, wheelIndex: number]>;
type SetRaycastVehicleBrakeMessage = WithUUID<'setRaycastVehicleBrake',
  [brake: number, wheelIndex: number]>;
type SetRaycastVehicleSteeringValueMessage = WithUUID<'setRaycastVehicleSteeringValue',
  [value: number, wheelIndex: number]>;
...
type ApplyForceMessage = WithUUID<'applyForce',
  [force: NgtTriplet, worldPoint: NgtTriplet]>;
type ApplyImpulseMessage = WithUUID<'applyImpulse',
  [impulse: NgtTriplet, worldPoint: NgtTriplet]>;
type ApplyLocalForceMessage = WithUUID<'applyLocalForce',
  [force: NgtTriplet, localPoint: NgtTriplet]>;
type ApplyLocalImpulseMessage = WithUUID<'applyLocalImpulse',
...
type WorldMessage<T extends WorldPropName> = Operation<SetOpName<T>,
  Required<NgtPhysicsStoreState[T]>>;
