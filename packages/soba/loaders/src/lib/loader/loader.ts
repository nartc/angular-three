import { defaultProjector, NgtComponentStore, skipFirstUndefined, tapEffect } from '@angular-three/core';
...
import { SobaProgress } from '../progress/progress';

interface SobaLoaderState {
    shown: boolean;
    loaderConfig: {
    ...
    styles: [
        ...
        `
            ...
            .inner-loader-data {
                ...
                color: #f0f0f0;
                font-size: 0.6em;
                font-family: -apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', 'Helvetica Neue', Helvetica, Arial,
                    Roboto, Ubuntu, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol';
                white-space: nowrap;
            }
        `,
    ],
...
export class SobaLoader extends NgtComponentStore<SobaLoaderState> implements AfterViewInit {
    private readonly sobaProgress = inject(SobaProgress);
    ...
    private readonly setShown = this.effect(
        tapEffect(() => {
            const shown = this.read((s) => s.shown);
            const active = this.sobaProgress.read((s) => s.active);

            if (shown !== active) {
                const sub = timer(300).subscribe(() => {
                    this.zone.run(() => {
                        this.write({ shown: active });
                    });
                });
    ...
    private readonly setProgress = this.effect(
        tapEffect(() => {
            const dataInterpolation = this.read((s) => s.loaderConfig.dataInterpolation);
            const progress = this.sobaProgress.read((s) => s.progress);
    ...
    ngAfterViewInit() {
        this.write({
            shown: this.sobaProgress.read((s) => s.active),
            loaderConfig: { dataInterpolation: (value: number) => `Loading ${value.toFixed(2)}%` },
        });
