import { NgtArgs, NgtReconstructOn } from '@angular-three/core';
...
import { SobaPerspectiveCamera } from '@angular-three/soba/cameras';
import { presetsObj, SobaContactShadows, SobaEnvironment } from '@angular-three/soba/staging';
...
<ngt-soba-orbit-controls [autoRotate]="true"></ngt-soba-orbit-controls>
`,
});

const presets = Object.keys(presetsObj);

Default.args = { background: true, preset: presets[0] };
Default.argTypes = { preset: { options: presets, control: { type: 'select' } } };
...
<ngt-soba-orbit-controls *reconstructOn="['camera']" [autoRotate]="true"></ngt-soba-orbit-controls>
<ngt-soba-perspective-camera [position]="[40, 40, 40]" [makeDefault]="true"></ngt-soba-perspective-camera>
    `,
});

Ground.args = { height: 15, radius: 60, preset: 'park' };
Ground.argTypes = {
    preset: { options: presets, control: { type: 'select' } },
    height: { control: createRangeControl(0, 50, 0.1) },
    radius: { control: createRangeControl(0, 200, 1) },
};
