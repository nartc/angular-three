...
function accumulativeApiFactory(shadows: NgtsAccumulativeShadows) {
    ...
    const api = {
        ...
        update: (frames = 1) => {
            ...
            const material = shadows.meshRef.nativeElement.material as SoftShadowMaterialProps & THREE.ShaderMaterial;
            if (!api.temporal) {
    ...
    Object.defineProperty(api, 'blend', {
        get: () => Math.max(2, shadows.get('frames') === Infinity ? shadows.get('blend') : shadows.get('frames')),
    });
...
@Component({
    ...
    template: `
        <ngt-group ngtCompound>
            ...
            <ngt-mesh receiveShadow [ref]="meshRef" [scale]="get('scale')" [rotation]="[-Math.PI / 2, 0, 0]">
                <ngt-plane-geometry />
    ...
    providers: [RxActionFactory, provideNgtsAccumulativeApi([NgtsAccumulativeShadows], accumulativeApiFactory)],
