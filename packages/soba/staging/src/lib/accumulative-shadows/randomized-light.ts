import { createInjectionToken, extend, injectNgtRef, NgtArgs, NgtRepeat, NgtRxStore } from '@angular-three/core';
...
function lightsApiFactory(lights: NgtsRandomizedLight) {
    ...
    lights.effect(
        combineLatest([lights.select(selectSlice(['radius', 'ambient', 'length', 'position'])), lights.groupRef.$]),
        ([, group]) => {
