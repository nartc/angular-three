...
export interface SobaBoundsSize {
    box: THREE.Box3;
    size: THREE.Vector3;
...
export interface SobaBoundsApi {
    getSize: () => SobaBoundsSize;
    refresh(object?: THREE.Object3D | THREE.Box3): SobaBoundsApi;
    clip(): SobaBoundsApi;
    fit(): SobaBoundsApi;
}
...
@Component({
    ...
    providers: [provideInstanceRef(SobaBounds, { compound: true })],
})
export class SobaBounds extends NgtCompound<NgtGroup> implements OnInit {
    @Input() set damping(damping: NgtObservableInput<number>) {
        this.write({ damping });
    }

    @Input() set fit(fit: NgtObservableInput<boolean>) {
        this.write({ fit });
    }

    @Input() set clip(clip: NgtObservableInput<boolean>) {
        this.write({ clip });
    }

    @Input() set observe(observe: NgtObservableInput<boolean>) {
        this.write({ observe });
    }

    @Input() set margin(margin: NgtObservableInput<number>) {
        this.write({ margin });
    }

    @Input() set eps(eps: NgtObservableInput<number>) {
        this.write({ eps });
    }

    @Output() fitChange = new EventEmitter<SobaBoundsSize>();
    ...
    private readonly preventDragHijacking = this.effect(
        tapEffect(() => {
            const controls = this.store.read((s) => s.controls) as unknown as ControlsProto;
    ...
    private readonly observeSizeChange = this.effect(
        tap(() => {
            const { clip, fit, observe } = this.read();
    ...
    private readonly setBeforeRender = this.effect<void>(
        tapEffect(() => {
            ...
            return internal.subscribe(({ delta }) => {
                if (this.current.animating) {
                    const { damping, eps } = this.read();
                    const controls = this.store.read((s) => s.controls) as unknown as ControlsProto;
                    const camera = this.store.read((s) => s.camera);
                    const invalidate = this.store.read((s) => s.invalidate);
    ...
    get api(): SobaBoundsApi {
        return {
            getSize: () => {
                const camera = this.store.read((s) => s.camera);
                const margin = this.read((s) => s['margin']);
            ...
            fit: () => {
                ...
                const { damping, margin } = this.read();
            ...
            refresh: (object?: THREE.Object3D | THREE.Box3) => {
                const { camera, controls } = this.store.read();

                if (isBox3(object)) this.box.copy(object);
                else {
                    const target = object || this.instanceRef.value;
