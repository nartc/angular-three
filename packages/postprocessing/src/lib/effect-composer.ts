...
export const [injectNgtpEffectComposertApi, provideNgtpEffectComposerApi] = createInjectionToken<NgtpEffectComposerApi>(
    'NgtpEffectComposert public API'
);
...
@Component({
    ...
    `,
    providers: [provideNgtpEffectComposerApi([NgtpEffectComposer], effectComposerApiFactory), RxActionFactory],
...
export class NgtpEffectComposer extends NgtRxStore implements OnInit {
    ...
    ngOnInit() {
        this.connect(
            'activeScene',
            combineLatest([this.select('scene').pipe(startWithUndefined()), this.#store.select('scene')]).pipe(
                map(([scene, defaultScene]) => scene || defaultScene)
            )
        );
        this.connect(
            'activeCamera',
            combineLatest([this.select('camera').pipe(startWithUndefined()), this.#store.select('camera')]).pipe(
                map(([camera, defaultCamera]) => camera || defaultCamera)
            )
        );
