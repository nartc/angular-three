...
- `@angular-three/soba`_: A set of helpers, utilities, and extra entities that are complimentary to `@angular-three/core`. Also, **soba** provides THREE.js entities that are from `three/examples/_` via [three-stdlib](https://github.com/pmndrs/three-stdlib) for better tree-shakability.
