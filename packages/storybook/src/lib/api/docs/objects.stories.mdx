...
Any THREE.js entities can be _referenced_ by NGT by the convention: `ngt-<kebab-case-entity-name>`. For example:
...
All NGT objects are exposed on the template via `exportAs` so we can assign a Template Variable to them. Most NGT objects have `(ready)` event which we can leverage to assign the object as soon as they are constructed and ready.
...
There are no recommended approaches as they are situational, and have both pros and cons to each.
