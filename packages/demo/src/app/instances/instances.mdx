...
This example shows 125000 spheres, spinning at 60fps. This is possible due to `THREE.InstancedMesh`. Technically, every mesh is a "draw call", which is a request to the GPU to draw a mesh. If we construct 125000 Meshes, that would be an insane amount of load for the GPU. Instead, we can leverage `THREE.InstancedMesh`, or Instancing, so we can send only one "draw call" to draw all the instances of the Mesh.
...
<Story id="examples-instances--default" height={480} />
