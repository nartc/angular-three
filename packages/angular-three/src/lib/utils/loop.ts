...
function render(timestamp: number, stateFactory: NgtStateFactory, frame?: XRFrame) {
...
export function createLoop<TCanvas>(roots: Map<TCanvas, NgtStateFactory>) {
    let running = false;
    let repeat: number;
    let frame: number;
    let state: NgtState;
    ...
    function invalidate(stateFactory?: NgtStateFactory, frames = 1): void {
    ...
    function advance(
        ...
        if (!stateFactory) for (const root of roots.values()) render(timestamp, root);
        else render(timestamp, stateFactory, frame);
