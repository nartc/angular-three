import type { NgtAnyRecord, NgtDomEvent, NgtEventManager, NgtEvents, NgtStateFactory } from './types';
...
export function createPointerEvents(stateFactory: NgtStateFactory): NgtEventManager<HTMLElement> {
    const { handlePointer } = createEvents(stateFactory);

    return {
        priority: 1,
        enabled: true,
        compute: (event: NgtDomEvent, rootFactory: NgtStateFactory) => {
