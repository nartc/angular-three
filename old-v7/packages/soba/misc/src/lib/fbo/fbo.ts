import { NgtComponentStore, NgtRef, NgtStore, tapEffect } from '@angular-three/core';
...
export class NgtSobaFBO extends NgtComponentStore<NgtSobaFBOState> {
  ...
  private readonly setTarget = this.effect<NgtSobaFBOParams>(
    tap(({ width, height, settings }) => {
      const targetRef = this.getState((s) => s.target);
      const { gl, size, viewport } = this.store.getState();
      ...
      this.set({ width: _width, height: _height, settings: _settings });
    })
  );
