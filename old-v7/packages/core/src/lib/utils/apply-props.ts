...
import type { NgtAnyConstructor, NgtAnyFunction, NgtInstanceLocalState, NgtUnknownRecord } from '../types';
...
export function diffProps<TInstance extends object>(
  instance: TInstance,
  props: NgtUnknownRecord,
  previousProps: NgtUnknownRecord = {},
  remove = false
) {
  ...
  const memoized: NgtUnknownRecord = { ...props };
...
export function applyProps<TInstance extends object>(instance: TInstance, props: NgtUnknownRecord): TInstance {
  // props is empty
  if (!Object.keys(props).length) return instance;

  // Filter equals, events and reserved props
  const localState = getInstanceLocalState(instance) || ({} as NgtInstanceLocalState);
  const rootState = localState.stateGetter?.();
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    const targetProp = (currentInstance as NgtUnknownRecord)[key] as NgtUnknownRecord;
    ...
    if (targetProp && targetProp['set'] && (targetProp['copy'] || targetProp instanceof THREE.Layers)) {
      ...
      (currentInstance as NgtUnknownRecord)[key] = value;
      // Auto-convert sRGB textures, for now ...
      // https://github.com/pmndrs/react-three-fiber/issues/344
      if (!rootState?.linear && (currentInstance as NgtUnknownRecord)[key] instanceof THREE.Texture) {
        ((currentInstance as NgtUnknownRecord)[key] as NgtUnknownRecord)['encoding'] = THREE.sRGBEncoding;
      }
    }
  ...
  if (
    ...
    rootState.internal &&
    (instance as NgtUnknownRecord)['raycast'] &&
    instanceHandlers !== localState.eventCount
  ) {
    // Pre-emptively remove the instance from the interaction manager
    rootState.removeInteraction((instance as NgtUnknownRecord)['uuid'] as string);
