...
export function removeInteractivity(stateGetter: NgtStateGetter, object: THREE.Object3D) {
...
export function createEvents(stateGetter: NgtStateGetter) {
  ...
  function intersect(event: NgtDomEvent, filter?: (objects: THREE.Object3D[]) => THREE.Object3D[]) {
    ...
    for (let i = 0; i < eventsObjects.length; i++) {
      const instanceState = getInstanceLocalState(eventsObjects[i])?.stateGetter();
      if (instanceState) {
    ...
    function handleRaycast(obj: THREE.Object3D<THREE.Event>) {
      const state = getInstanceLocalState(obj)?.stateGetter();
    ...
    let hits: THREE.Intersection<THREE.Object3D<THREE.Event>>[] = eventsObjects
      ...
      .sort((a, b) => {
        const aState = getInstanceLocalState(a.object)?.stateGetter();
        const bState = getInstanceLocalState(b.object)?.stateGetter();
    ...
    for (const hit of hits) {
      ...
      // Bubble event up
      while (eventObject) {
        if (getInstanceLocalState(eventObject)?.eventCount) intersections.push({ ...hit, eventObject });
  ...
  function handleIntersects(
    ...
    if (intersections.length) {
      ...
      for (const hit of intersections) {
        const state = getInstanceLocalState(hit.object)?.stateGetter() || rootState;
