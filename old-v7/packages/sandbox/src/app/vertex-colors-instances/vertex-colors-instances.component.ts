...
class Instances {
  ...
  private readonly colors = Array.from({ length: this.length }, () => niceColor[Math.floor(Math.random() * 5)]);

  readonly colorArray = Float32Array.from(
...
@Component({
  ...
  `,
  imports: [NgtAmbientLight, NgtDirectionalLight, NgtSobaOrbitControls, Instances],
})
class Scene {}
