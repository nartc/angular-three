...
**Angular Three** is an [Angular](https://angular.io) wrapper for [THREE.js](https://threejs.org). This project is inspired by [react-three-fiber](https://github.com/pmndrs/react-three-fiber).

Different from **React Three Fiber** which is a **Renderer**, **Angular Three** is a wrapper because it wraps **THREE.js** classes in **Directives** to take advantage of **Angular** template and other **Angular** features.
...
**Angular Three** relies heavily on
...
`<ngt-mesh>` before we would write `<ngt-boxBufferGeometry>` and
`<ngt-meshBasicMaterial>`, and we also nest the `Geometry` with
