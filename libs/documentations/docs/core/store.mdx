...
All `NgtInstance` injects `NgtStore` so all NGT objects essentially have access to the Canvas state object.
...
export class CubeComponent extends NgtInstance {
    ngOnInit() {
        ...
        const unregister =
            this.store.registerBeforeRender(/* NgtBeforeRenderRecord */);
