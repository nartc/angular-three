...
export class NgtPortalContent {
    constructor(
        public templateRef: TemplateRef<{ portal: Ref<THREE.Scene> }>
    ) {}

    static ngTemplateContextGuard(
        dir: NgtPortalContent,
        ctx: any
    ): ctx is { portal: Ref<THREE.Scene> } {
        return true;
    }
}
...
@Component({
    ...
    providers: [
        ...
        },
        {
            ...
            useFactory: (portal: NgtPortal) => {
                return () =>
                    portal.get((s) => s.portalStore).get((s) => s.sceneRef);
            },
            deps: [NgtPortal],
        },
        {
        ...
        },
        {
            ...
            useFactory: (portal: NgtPortal) => {
                return () =>
                    portal.get((s) => s.portalStore).get((s) => s.cameraRef);
            },
            deps: [NgtPortal],
        },
    ],
...
export class NgtPortal extends NgtInstance<THREE.Scene, NgtPortalState> {
    ...
    private readonly initPortalStore = this.effect<void>(
        tap(() => {
            ...
            const { regress: _, ...performanceOptions } =
    ...
    private readonly subscribeToRoot = this.effect<void>(
        tapEffect(() => {
            const unsub = this.store.select().subscribe((root) => {
                this.get((s) => s.portalStore).set((portal) =>
                    this.inject(root, portal)
                );
            });
    ...
    private readonly setInitialPortalState = this.effect<void>(
        tap(() => {
            this.get((s) => s.portalStore).set((portal) =>
                this.inject(this.store.get(), portal)
            );
        })
    );
    ...
    private inject(root: NgtState, injected: NgtState): Partial<NgtState> {
        ...
        if (injected && size) {
            ...
            viewport = root.viewport.getCurrentViewport(
                camera,
                new THREE.Vector3(),
                size
            );
