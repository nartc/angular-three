...
export function createEvents(stateGetter: NgtStateGetter) {
  /** Calculates delta */
  ...
  function filterPointerEvents(objects: THREE.Object3D[]) {
    return objects.filter((obj) =>
      ['move', 'over', 'enter', 'out', 'leave'].some((name) => {
        ...
        return getInstanceInternal(obj)?.handlers[eventName];
      })
    );
  }
  ...
  function intersect(
    ...
    for (let i = 0; i < eventsObjects.length; i++) {
      const instanceState = getInstanceInternal(
        eventsObjects[i]
      )?.stateGetter();
      if (instanceState) {
    ...
    function handleRaycast(obj: THREE.Object3D<THREE.Event>) {
      const state = getInstanceInternal(obj)?.stateGetter();
    ...
    let hits: THREE.Intersection<THREE.Object3D<THREE.Event>>[] = eventsObjects
      ...
      .sort((a, b) => {
        const aState = getInstanceInternal(a.object)?.stateGetter();
        const bState = getInstanceInternal(b.object)?.stateGetter();
    ...
    for (const hit of hits) {
      ...
      // Bubble event up
      while (eventObject) {
        if (getInstanceInternal(eventObject)?.eventCount)
    ...
    if (
      ...
      for (const captureData of state.internal.capturedMap
        .get(event.pointerId)!
        .values()) {
  ...
  function handleIntersects(
    ...
    const rootState = stateGetter();
    // If anything has been found, forward it to the event listeners
    if (intersections.length) {
      const localState = { stopped: false };
      for (const hit of intersections) {
        const state =
          getInstanceInternal(hit.object)?.stateGetter() || rootState;
  ...
  function cancelPointer(intersections: NgtIntersection[]) {
    ...
    for (const hoveredObj of internal.hovered.values()) {
      ...
      if (
        ...
        const instance = getInstanceInternal(eventObject);
  ...
  function pointerMissed(event: MouseEvent, objects: THREE.Object3D[]) {
    for (let i = 0; i < objects.length; i++) {
      const instance = getInstanceInternal(objects[i]);
  ...
  function handlePointer(name: string) {
    ...
    return function handleEvent(event: NgtDomEvent) {
      const { onPointerMissed, internal } = stateGetter();
