...
import { NGT_HOST_PARENT_OBJECT, NGT_PARENT_OBJECT } from '../di/parent-object';
...
export class NgtObjectController extends Controller implements OnDestroy {
    ...
    constructor(
        ...
        @Optional()
        @SkipSelf()
        @Inject(NGT_HOST_PARENT_OBJECT)
        private hostParentObjectFn: AnyFunction
    ) {
        super();
    }
            ...
            this.initSubscription = this.store.onCanvasReady(
                ...
                () => {
                    ...
                    if (this.object) {
                        ...
                        if (
                            this.objectInputsController.appendMode !== 'none' &&
    ...
    private addToParent() {
        let parentObject = (this.hostParentObjectFn?.() ||
            this.parentObjectFn?.()) as THREE.Object3D;
    ...
    private remove() {
        if (this.objectInputsController.appendTo) {
            ...
            this.parentObjectFn?.() &&
            this.parentObjectFn()?.uuid !== this.object.uuid &&
            this.objectInputsController.appendMode === 'immediate'
        ) {
            this.parentObjectFn().remove(this.object);
        } else {
            const scene = this.canvasStore.get((s) => s.scene);
            if (scene) {
