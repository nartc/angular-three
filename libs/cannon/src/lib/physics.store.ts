...
export class NgtPhysicsStore extends NgtComponentStore<NgtPhysicsState> {
    ...
    private readonly initWorker = this.effect<WorkerInputs>(
        tapEffect((physicsInputs) => {
            ...
            (worker as unknown as EventEmitter).on(
                'collide',
                this.collideHandler.bind(this)
            );
            (worker as unknown as EventEmitter).on(
                'collideBegin',
            ...
            (worker as unknown as EventEmitter).on(
                'frame',
                this.frameHandler.bind(this)
            );
            (worker as unknown as EventEmitter).on(
                'rayhit',
                this.rayhitHandler.bind(this)
            );

            return () => {
                worker.terminate();
                (
                    worker as unknown as { removeAllListeners: () => void }
                ).removeAllListeners();
            };
        })
    );
    ...
    private readonly collideBeginHandler = this.effect<
    ...
    private readonly collideEndHandler = this.effect<
