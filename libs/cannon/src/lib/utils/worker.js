...
self.onmessage = (e) => {
    ...
    switch (op) {
        case 'init': {
            ...
            state.world.broadphase = new (broadphases[
            ...
            Object.assign(
                state.world.defaultContactMaterial,
                defaultContactMaterial
            );
        ...
        case 'step': {
            ...
            for (const id of Object.keys(state.subscriptions)) {
                ...
                if (value instanceof Vec3 || value instanceof Quaternion)
                    value = value.toArray();
                        ...
                        ({ type, body, target, contact }) => {
                            ...
                            self.postMessage({
                                ...
                                collisionFilters: {
                                    ...
                                    targetFilterGroup:
                                        target.collisionFilterGroup,
                                    targetFilterMask:
                                        target.collisionFilterMask,
                                },
                            });
                        }
                    );
        ...
        case 'setPosition':
            ...
            break;
        case 'setQuaternion':
            state.bodies[uuid].quaternion.set(
                props[0],
                props[1],
                props[2],
                props[3]
            ...
            break;
        case 'setRotation':
            state.bodies[uuid].quaternion.setFromEuler(
                props[0],
                props[1],
                props[2]
            );
        ...
        case 'setAngularVelocity':
            state.bodies[uuid].angularVelocity.set(
                props[0],
                props[1],
                props[2]
            );
        ...
        case 'setMass':
            ...
            if (props !== 0 && state.bodies[uuid].type === 0)
        ...
        case 'setIterations':
            ...
            break;
        case 'setBroadphase':
            state.world.broadphase = new (broadphases[props + 'Broadphase'] ||
                NaiveBroadphase)(state.world);
            break;
        case 'setAxisIndex':
            state.world.broadphase.axisIndex =
                props === undefined || props === null ? 0 : props;
            break;
        case 'applyForce':
            state.bodies[uuid].applyForce(
                new Vec3(...props[0]),
                new Vec3(...props[1])
            );
            break;
        case 'applyImpulse':
            state.bodies[uuid].applyImpulse(
                new Vec3(...props[0]),
                new Vec3(...props[1])
            );
            break;
        case 'applyLocalForce':
            state.bodies[uuid].applyLocalForce(
                new Vec3(...props[0]),
                new Vec3(...props[1])
            );
            break;
        case 'applyLocalImpulse':
            state.bodies[uuid].applyLocalImpulse(
                new Vec3(...props[0]),
                new Vec3(...props[1])
            );
            break;
        ...
        case 'addSpring': {
            ...
            worldAnchorA = Array.isArray(worldAnchorA)
                ? new Vec3(...worldAnchorA)
                : undefined;
            worldAnchorB = Array.isArray(worldAnchorB)
                ? new Vec3(...worldAnchorB)
                : undefined;
            localAnchorA = Array.isArray(localAnchorA)
                ? new Vec3(...localAnchorA)
                : undefined;
            localAnchorB = Array.isArray(localAnchorB)
                ? new Vec3(...localAnchorB)
                : undefined;
        ...
        case 'addRay': {
            ...
            const ray = new Ray(
                from ? new Vec3(...from) : undefined,
                to ? new Vec3(...to) : undefined
            );
        ...
        case 'addRaycastVehicle': {
            ...
            for (let i = 0; i < wheelInfos.length; i++) {
                ...
                wheelInfo.directionLocal = new Vec3(
                    ...wheelInfo.directionLocal
                );
                wheelInfo.chassisConnectionPointLocal = new Vec3(
            ...
            (vehicle.postStep = () => {
                for (
                    let i = 0;
                    i < state.vehicles[uuid].wheelInfos.length;
                    i++
                ) {
            ...
            state.world.addEventListener(
                'preStep',
                state.vehicles[uuid].preStep
            );
            state.world.addEventListener(
                'postStep',
                state.vehicles[uuid].postStep
            );
            break;
        }
