...
export class NgtSobaCameraShake extends NgtInstance<
    {},
    NgtSobaCameraShakeState
> {
    ...
    private readonly configureChangeEvent = this.effect<{}>(
        tapEffect(() => {
            ...
            if (controls) {
                ...
                const callback = () =>
                    void initialRotation.set(camera.rotation.clone());
                controls.addEventListener('change', callback);
                callback();
                return () =>
                    void controls.removeEventListener('change', callback);
            }
            return;
        })
    );
