...
const isBox3 = (def: unknown): def is THREE.Box3 =>
            ...
            const unregister = this.store.registerBeforeRender({
                callback: ({ delta }) => {
                    if (this.current.animating) {
                        ...
                        this.current.zoom = THREE.MathUtils.damp(
                            this.current.zoom,
                            this.goal.zoom,
                            damping,
                            delta
                        ...
                        if (
                            ...
                            !(
                                Math.abs(this.current.zoom - this.goal.zoom) <
                                eps
                            )
                        )
                            return;
                        ...
                        if (
                            controls &&
                            !equals(this.current.focus, this.goal.focus, eps)
                        )
                            return;
    ...
    get api(): NgtSobaBoundsApi {
        return {
            getSize: () => {
                ...
                const distance =
                    margin * Math.max(fitHeightDistance, fitWidthDistance);
            ...
            fit: () => {
                ...
                if (is.orthographic(camera)) {
                    ...
                    maxWidth *= 2;
                    const zoomForHeight =
                        (camera.top - camera.bottom) / maxHeight;
                    const zoomForWidth =
                        (camera.right - camera.left) / maxWidth;
                    this.goal.zoom =
                        Math.min(zoomForHeight, zoomForWidth) / margin;
                    if (!damping) {
            ...
            refresh: (object?: THREE.Object3D | THREE.Box3) => {
                ...
                if (
                    ...
                    const direction = camera.position
                        ...
                        .normalize()
                        .multiplyScalar(distance);
                    const newPos = (controls as unknown as ControlsProto).target
                        .clone()
                        .add(direction);
