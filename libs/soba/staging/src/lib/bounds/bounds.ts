...
@Component({
  ...
  providers: [
    provideNgtObject(NgtSobaBounds),
    provideObjectRef(NgtSobaBounds),
    provideObjectHostRef(NgtSobaBounds),
  ],
})
export class NgtSobaBounds extends NgtGroup {
  ...
  readonly #preventDragHijacking = this.effect(
    tapEffect(() => {
  ...
  readonly #observeSizeChange = this.effect(
    tap(() => {
  ...
  override postInit(): void {
    ...
    this.#observeSizeChange(
      this.select(
        ...
        this.instance.pipe(filter((val) => !!val))
      )
    );
