            ...
            this.onCanvasReady(this.store.ready$, () => {
                this.set(
                    this.select(
                        this.select((s) => s['angle']),
                        (angle) => {
                            const radiusBottomExplicit = this.get(
                                (s) => s['radiusBottomExplicit']
                            );
                            const radiusBottom = this.get(
                                (s) => s['radiusBottom']
                            );
                            return {
                                radiusBottom: radiusBottomExplicit
                                    ? radiusBottom
                                    : angle * 7,
                            };
                        }
            ...
            const unregister = this.store.registerBeforeRender({
                callback: () => {
                    ...
                    if (material && mesh) {
                        ...
                        if (mesh.parent) {
                            mesh.lookAt(
                                mesh.parent.target.getWorldPosition(vec)
                            );
                        }
                    }
                },
