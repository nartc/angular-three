...
@Component({
    ...
    template: `
        <ngt-points
            ...
            (beforeRender)="onBeforeRender($event.state)"
        >
            <ngt-buffer-geometry>
                <ng-container
                    *ngIf="bufferAttributes$ | async as bufferAttributes"
                >
...
> {
    ...
    onBeforeRender(state: NgtRenderState) {
        if (this.materialRef.value) {
            this.materialRef.value.uniforms['time'].value =
                state.clock.getElapsedTime() * this.get((s) => s.speed);
        }
    }
}
