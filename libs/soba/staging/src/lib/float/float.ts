                    ...
                    const unregister = this.store.registerBeforeRender({
                        callback: ({ clock }) => {
                            ...
                            if (this.instance.value) {
                                this.instance.value.rotation.x =
                                    (Math.cos((t / 4) * speed) / 8) *
                                    rotationIntensity;
                                this.instance.value.rotation.y =
                                    (Math.sin((t / 4) * speed) / 8) *
                                    rotationIntensity;
                                this.instance.value.rotation.z =
                                    (Math.sin((t / 4) * speed) / 20) *
                                    rotationIntensity;
                                this.instance.value.position.y =
                                    (Math.sin((t / 4) * speed) / 10) *
                                    floatIntensity;
                            }
                        },
                    });
