...
const makePositionVector = (xyz: number[]) =>
    makeVector3(xyz as NgtVector3).multiplyScalar(0.38);
...
export interface NgtSobaGizmoViewCubeGenerticState<TPart extends object>
    extends NgtInstanceState<TPart> {
    font: string;
    opacity: number;
    color: string;
    hoverColor: string;
...
@Directive()
export abstract class NgtSobaGizmoViewCubeGeneric<
    TPart extends object
> extends NgtInstance<TPart, NgtSobaGizmoViewCubeGenerticState<TPart>> {
...
export class NgtSobaGizmoViewCubePassThrough {
    ...
    constructor(
        @Self() @Optional() private host: NgtSobaGizmoViewCubeGeneric<any>
    ) {
        if (!host) return;
    }
    ...
    private assertWrapper(
        wrapper: unknown
    ): asserts wrapper is NgtSobaGizmoViewCubeGeneric<any> {
        if (!(wrapper instanceof NgtSobaGizmoViewCubeGeneric)) {
            throw new Error(
                `[NgtSobaGizmoViewCubePassThrough] wrapper is not an NgtSobaGizmoViewCubeGeneric`
            );
        }
    }
}
...
export class NgtSobaGizmoFaceMaterial extends NgtSobaGizmoViewCubeGeneric<THREE.MeshLambertMaterial> {
    ...
    readonly texture$ = this.select(
        ...
        () => {
            ...
            const { color, strokeColor, textColor, font, faces, index } =
                this.get();
