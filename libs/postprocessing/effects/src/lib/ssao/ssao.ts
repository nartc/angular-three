...
export class NgtSSAO extends NgtCommonEffect<SSAOEffect> {
    static ngAcceptInputType_options:
        | ConstructorParameters<AnyConstructor<SSAOEffect>>[2]
        | undefined;
    ...
    protected override adjustCtorParams(instanceArgs: unknown[]): unknown[] {
        const { camera, normalPass, depthDownSamplingPass, resolutionScale } =
            this.effectComposer.get();
        ...
        return [
            ...
            normalPass && !depthDownSamplingPass ? normalPass.texture : null,
            {
                ...
                intensity: 1.0,
                color: null,
                normalDepthBuffer: depthDownSamplingPass
