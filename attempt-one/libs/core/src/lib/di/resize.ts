...
export const [injectResizeOptions, provideResizeOptions, NGT_RESIZE_OPTIONS] =
  createInjection<NgtResizeOptions>('ngtResize Options', {
    defaultValueOrFactory: defaultResizeOptions,
    provideValueFactory: (value) => ({ ...defaultResizeOptions, ...value }),
  });

export const [injectResizeObserverSupport] = createInjection<boolean>(
