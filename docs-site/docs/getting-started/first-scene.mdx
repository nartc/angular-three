...
@Component({
  ...
  template: `
    <ngt-canvas>
      <ngt-mesh (animateReady)="onAnimateReady($event.animateObject)">
...
If we save now and check our application, we will see **nothing**. It is because we don't have any **Objects** added to our scene yet. Let's add one in the next step.

## Adding a Mesh

A `Mesh` can be visualized as a physical object in 3D space. It can hold any type of shape (**Geometry**) and have any type of texture on its surface (**Material**) to represent the object. In **THREE.js**, `Mesh` is the most basic object, and it is a great starting point.

Beside the `CanvasComponent`, **NGT** represents everything else in **THREE.js** via **Directive** as wrappers. These wrappers' jobs are to instantiate the respective **THREE.js** classes without the need of any templates, hence **Directive**. When we use a **Directive** on the template, we instantiate the **Directive** (unavoidable), and the **THREE.js** class itself.
...
The convention is every class in **THREE.js** is available as a **Directive** with `kebab-case-THREE-class-name` prefixed with `ngt-` as selector.
...
:::info
**NGT** is built with **SCAM** (Single Component as Module) in mind hence almost each wrapper will be available in its own **Module**. This allows tree-shaking to efficient remove anything you're not using from **NGT**.
:::

With that, let's import `ThreeMeshModule`, as well as `ThreeBoxBufferGeometryModule`, and `ThreeMeshBasicMaterialModule` in the `AppModule` to enable `<ngt-mesh>`, `<ngt-box-geometry>`, and `<ngt-mesh-basic-material>`
...
We will run into various other types of objects from **THREE.js**. They will be exported from appropriate modules as well.
:::
...
Our application will now show a gray-ish square (it is actually a **cube** because we are in 3D space) on the screen.

:::note
For demos throughout the documentation, [Stackblitz](https://stackblitz.com) will be used, just like the embedded below.
:::
...
When we place `<ngt-mesh>` inside of `<ngt-canvas>`, the `Mesh`, which is created by `MeshDirective`, is added to the `Scene` _automatically_ (`scene.add(mesh)`). Same concept with `<ngt-box-geometry>` and `<ngt-mesh-basic-material>`, the underlined `Geometry` and `Material` are added to the `Mesh` _automatically_. This is possible thanks to **Angular Dependency Injection**.
