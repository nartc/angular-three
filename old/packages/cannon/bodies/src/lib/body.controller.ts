...
export class NgtCannonBodyController extends Controller implements OnInit {
  ...
  private cannonPropsChanges$ = combineLatest([
    this.store.select('getPhysicProps').pipe(startWithUndefined()),
    this.objectInputsController.changes$.pipe(stateful(startWith({}))),
  ]).pipe(
  ...
  override ngOnInit() {
    ...
    zonelessRequestAnimationFrame(() => {
      ...
      this.store.effect(this.cannonPropsChanges$, this.initWorker);
  ...
  private initWorker = () => {
    ...
    const object = this.store.get('object3d') || new THREE.Object3D();
  ...
  private getByIndex(index: number): BodyProps & NgtObject3dProps {
    const getPhysicProps = this.store.get('getPhysicProps');
